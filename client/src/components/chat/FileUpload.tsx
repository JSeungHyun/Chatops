...
const ACCEPT_TYPES = 'image/jpeg,image/png,image/gif,image/webp,video/mp4,video/webm,video/quicktime,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,application/zip';
...
export function FileUpload({ onFileUploaded, disabled, roomId }: FileUploadProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > limit) {
      setError(isVideo ? '동영상 크기는 50MB를 초과할 수 없습니다' : '파일 크기는 10MB를 초과할 수 없습니다');
